...
if [ "$1" ]; then
    ...
    if [ -f $file_path ]; then
        ...
        if [[ $file_name == *"OFFICIAL"* ]] || [[ $FORCE_JSON == 1 ]]; then
            ...
            build_type=$(grep ro\.everest\.build\.type ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            base_version=$(grep ro\.everest\.base\.version ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
